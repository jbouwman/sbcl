...
(with-test (:name (:fiber :gc-triggered-by-fiber-allocation))
  ;; Each fiber allocates ~6 MB of conses in one call -- enough to
  ;; trip the default GC threshold mid-allocation -- and we run many
  ;; of them so freshly mmap'd and freelist-recycled fiber stacks
  ;; both get exercised.
  (flet ((heavy ()
           (let ((acc nil))
             (dotimes (i 100000)
...
;;; Surfaced under load (an HTTP-server example crashed around request
;;; 1300).  Fixed by keying the Lisp wrapper through
...
(with-test (:name (:fiber :thread-exit-releases-orphaned-fibers))
  ;; A thread that exits without DESTROY-FIBERing its children must
  ;; not leak the per-fiber mmap'd stacks (~160 KiB VA each).  We
  ;; spawn-and-exit many such threads in series; if the cleanup
  ;; doesn't fire we'd accumulate ~64 MiB of VA per round and OOM
  ;; or fail the VA assertion below.
  (let ((rounds 50)
        (fibers-per-thread 200)
        (start-vsz (sb-ext:get-bytes-consed))) ; rough proxy
    (declare (ignorable start-vsz))
    (dotimes (round rounds)
      (sb-thread:join-thread
       (sb-thread:make-thread
        (lambda ()
          ;; Create a main fiber + N child fibers, then return without
          ;; destroying them.  The C runtime should release them when
          ;; the thread struct is freed.
          (let ((m (make-main-fiber)))
            (declare (ignorable m))
            (dotimes (i fibers-per-thread)
              (declare (ignore i))
              (make-fiber (lambda () (loop (fiber-yield)))))
            ;; Deliberately return without destroying.
            ))
        :name (format nil "leak-~D" round))))
    (sb-ext:gc :full t)
    (assert t)))

(with-test (:name (:fiber :concurrent-switching :current-fiber-tls))
  ;; Regression: *CURRENT-FIBER* must be per-thread.  Two threads each
  ;; ping-ponging between their own main and child must not see each
  ;; other's fiber via *CURRENT-FIBER* through the shared global cell.
  ;; Pre-fix, this raced within a few hundred switches and surfaced as
  ;; "FROM fiber is owned by a different thread" inside FIBER-YIELD.
  (let* ((switches-per-thread 5000)
         (n-threads 4)
           ...
           (loop for i below n-threads collect
                 (let ((idx i))
                   (sb-thread:make-thread
                    (lambda ()
                      (let ((m (make-main-fiber))
                            (c (make-fiber (lambda () (loop (fiber-yield))))))
                        (sb-thread:wait-on-semaphore start)
                        (handler-case
                            (progn (dotimes (k switches-per-thread)
                                     (fiber-switch m c))
                                   (setf (aref results idx) :ok))
                          (error (e) (setf (aref results idx) e)))
                        (destroy-fiber c)
                        (destroy-fiber m)))
                    :name (format nil "fiber-tls-~D" idx))))))
    (sleep 0.05)
    (dotimes (_ n-threads) (sb-thread:signal-semaphore start))
    (mapc #'sb-thread:join-thread threads)
    (let ((failures (loop for r across results
                          unless (eq r :ok) collect r)))
      (assert (null failures) ()
              "Cross-thread current-fiber leak: ~D thread(s) errored: ~S"
              (length failures) failures))))

(defvar *fbso-a*) (defvar *fbso-b*)
(defvar *fbso-c*) (defvar *fbso-d*)

(with-test (:name (:fiber :binding-stack-overflow :recoverable))
  ;; Pushing past the fiber's 8 KiB binding stack must raise
  ;; SB-KERNEL:BINDING-STACK-EXHAUSTED (a STORAGE-CONDITION) rather
  ;; than crash the process -- parity with the thread BS guard.
  ;; Toggle lose_on_corruption_p (set by --lose-on-corruption in the
  ;; test runner) the same way the CS-overflow test above does.
  (let* ((main (make-main-fiber))
         (outcome nil)
         (saved-flag %lose-on-corruption-p)
         (child
          (make-fiber
           (lambda ()
             ...
             (labels ((recur (depth)
                        ...
                        ;; 4 bindings per progv * many frames =
                        ;; ~128 frames to overflow 8 KiB BS while
                        ;; using ~4 KiB CS -- comfortably budgeted.
                        (progv '(*fbso-a* *fbso-b* *fbso-c* *fbso-d*)
                               '(0 0 0 0)
                          (recur (1+ depth)))))
               (handler-case
                   (recur 0)
    ...
    (unwind-protect
         (progn
           ...
           (fiber-switch main child))
      (setf %lose-on-corruption-p saved-flag))
    (assert (eq outcome :caught)
            () "expected BINDING-STACK-EXHAUSTED to be caught, got ~S" outcome)
    (destroy-fiber child)
    (destroy-fiber main)))
