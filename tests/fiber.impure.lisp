...
;;; Main fibers that stay registered between uses and are installed
;;; with WITH-CURRENT-FIBER only while the thread has no current fiber,
;;; as a generator library does to run on a plain thread.  Worker A is
;;; resumed both from the top level, under its own host, and from
;;; inside worker B, from B.  Installing A's host there instead, over
;;; the running B, gave the host B's stack pointers, and the next switch
;;; away from the host faulted.

(defun call-hosted (host-cell thunk)
  (if (current-fiber)
      (funcall thunk)
      (with-current-fiber ((or (car host-cell)
                               (setf (car host-cell)
                                     (make-main-fiber :current nil))))
        (funcall thunk))))

(with-test (:name (:fiber :with-current-fiber :worker-resumed-inside-worker))
  (let* ((host-a (list nil))
         (host-b (list nil))
         (a (call-hosted host-a
                         (lambda ()
                           (make-fiber (lambda ()
                                         (loop for i from 0
                                               do (yield-fiber i)))))))
         (b (call-hosted host-b
                         (lambda ()
                           (make-fiber
                            (lambda ()
                              (loop
                                (let ((junk (make-list 1000)))
                                  (yield-fiber
                                   (call-hosted host-a
                                                (lambda () (resume-fiber a))))
                                  (sb-ext:gc :full t)
                                  (assert (= 1000 (length junk)))))))))))
    (dotimes (i 100)
      (assert (= (* 2 i)
                 (call-hosted host-b (lambda () (resume-fiber b)))))
      (assert (= (1+ (* 2 i))
                 (call-hosted host-a (lambda () (resume-fiber a))))))
    (assert (null (current-fiber)))
    (mapc #'release-fiber (list a b (car host-a) (car host-b)))))
