...
(with-test (:name (:fiber :make-main-fiber :current-nil))
  (let ((main (make-main-fiber :current nil)))
    (unwind-protect
         (progn
           (assert (null (current-fiber)))
           (assert (eq :running (fiber-state main)))
           (assert (eq sb-thread:*current-thread* (fiber-thread main))))
      (release-fiber main))))

(with-test (:name (:fiber :make-main-fiber :refused-with-current-fiber))
  ;; Both on the thread's stack under its main fiber, and on a worker's.
  (with-main+child (main child (lambda ()
                                 (handler-case (make-main-fiber :current nil)
                                   (current-fiber-error (c) c))))
    (let ((c (handler-case (make-main-fiber)
               (current-fiber-error (c) c))))
      (assert (eq main (fiber-error-fiber c)))
      (assert (eq 'make-main-fiber (current-fiber-error-operation c))))
    (let ((c (switch-fiber main child)))
      (assert (typep c 'current-fiber-error))
      (assert (eq child (fiber-error-fiber c))))))

(with-test (:name (:fiber :with-current-fiber))
  (let ((main (make-main-fiber :current nil)))
    (unwind-protect
         (progn
           (with-current-fiber (main)
             (assert (eq main (current-fiber)))
             (with-current-fiber (main)
               (assert (eq main (current-fiber))))
             (assert (eq main (current-fiber)))
             (with-fiber (f (lambda () (yield-fiber (current-fiber)) :done))
               (assert (eq f (resume-fiber f)))
               (assert (eq main (current-fiber)))
               (assert (eq :done (resume-fiber f)))))
           (assert (null (current-fiber)))
           (block nil
             (with-current-fiber (main)
               (return)))
           (assert (null (current-fiber)))
           ;; Still registered: it can be installed again.
           (with-current-fiber (main)
             (with-fiber (f (lambda () 42))
               (assert (eql 42 (join-fiber f))))))
      (release-fiber main))))

(with-test (:name (:fiber :with-current-fiber :released-in-body))
  (let ((main (make-main-fiber :current nil))
        (replacement nil))
    (with-current-fiber (main)
      (release-fiber main)
      (assert (null (current-fiber)))
      (setf replacement (make-main-fiber)))
    ;; The exit leaves alone a fiber BODY made current after the release.
    (assert (eq replacement (current-fiber)))
    (release-fiber replacement)))

(with-test (:name (:fiber :with-current-fiber :errors))
  (let ((released (make-main-fiber :current nil)))
    (release-fiber released)
    (assert-error (with-current-fiber (released)) dead-fiber-error))
  (let ((foreign (sb-thread:join-thread
                  (sb-thread:make-thread
                   (lambda () (make-main-fiber :current nil))))))
    (let ((c (handler-case (with-current-fiber (foreign))
               (fiber-thread-mismatch-error (c) c))))
      (assert (eq foreign (fiber-error-fiber c)))
      (assert (eq :current (fiber-thread-mismatch-error-role c)))))
  (let* ((host (make-main-fiber :current nil))
         (worker (with-current-fiber (host)
                   (make-fiber (lambda ())))))
    ;; Only a switch makes a worker current.
    (assert-error (with-current-fiber (worker)))
    ;; Nor can a main fiber be installed over a worker that is running.
    (with-main+child (main child (lambda ()
                                   (handler-case (with-current-fiber (host)
                                                   :unreachable)
                                     (current-fiber-error (c) c))))
      (let ((c (switch-fiber main child)))
        (assert (typep c 'current-fiber-error))
        (assert (eq child (fiber-error-fiber c)))
        (assert (eq 'with-current-fiber (current-fiber-error-operation c)))))
    (assert (null (current-fiber)))
    (release-fiber worker)
    (release-fiber host)))
