...
(with-test (:name (:fiber :yield :basic))
  (let* ((main (make-main-fiber))
         (steps '())
         (child (make-fiber
                 (lambda ()
                   (push :child-start steps)
                   (fiber-yield)
                   (push :child-resumed steps)
                   (fiber-yield)
                   (push :child-end steps)))))
    (fiber-switch main child)
    (push :main-after-first steps)
    (fiber-switch main child)
    (push :main-after-second steps)
    (fiber-switch main child)
    (assert (equal (nreverse steps)
                   '(:child-start :main-after-first
                     :child-resumed :main-after-second
                     :child-end)))
    (destroy-fiber child)
    (destroy-fiber main)))

(with-test (:name (:fiber :yield :no-return-fiber-errors))
  ;; A freshly-made main fiber has never been resumed; FIBER-YIELD
  ;; from it has no destination.
  (let ((main (make-main-fiber)))
    (assert-error (fiber-yield))
    (destroy-fiber main)))

;;; --- Catch / throw isolation across fiber boundary ---

(with-test (:name (:fiber :throw-cannot-escape-fiber))
  ;; THROW from inside a fiber must not unwind to a CATCH established
  ;; on the caller's stack: that would re-enter the caller's frames
  ;; from a different stack and corrupt the unwinder.  FIBER-SWITCH
  ;; saves/restores *CURRENT-CATCH-BLOCK*, so a fresh fiber starts
  ;; with no visible catches.
  (let* ((main (make-main-fiber))
         (peer-error nil)
         (peer (make-fiber
                (lambda ()
                  (handler-case (throw 'outer-tag :should-not-reach)
                    (control-error (e) (setf peer-error e)))))))
    (catch 'outer-tag
      (fiber-switch main peer))
    (assert (typep peer-error 'control-error)
            () "expected control-error on throw without matching catch, got ~S"
            peer-error)
    (destroy-fiber peer)
    (destroy-fiber main)))

(with-test (:name (:fiber :throw-stays-within-fiber))
  ;; A fiber-internal CATCH with a matching THROW works normally.
  (let* ((main (make-main-fiber))
         (got nil)
         (peer (make-fiber
                (lambda ()
...
(with-test (:name (:fiber :pin :no-current-fiber))
  ;; WITH-FIBER-PINNED errors when *CURRENT-FIBER* is NIL.  Bind to
  ;; NIL explicitly to avoid pollution from prior main fibers in this
  ;; test run.
  (let ((sb-fiber::*current-fiber* nil))
    (assert-error (with-fiber-pinned () 42))))

(with-test (:name (:fiber :pin :predicate))
  (let ((main (make-main-fiber)))
    ...
    (with-fiber-pinned ()
      ...
      (with-fiber-pinned ()
        (assert (fiber-pinned-p main)))
      ;; Outer pin still in effect after inner exits.
      (assert (fiber-pinned-p main)))
    (assert (not (fiber-pinned-p main)))
    (destroy-fiber main)))

(with-test (:name (:fiber :pin :switch-from-pinned-errors))
  (let* ((main (make-main-fiber))
         (child (make-fiber (lambda ()))))
    (with-fiber-pinned ()
      (assert-error (fiber-switch main child)))
    ;; Pin released on normal exit -- switch should work again.
    (fiber-switch main child)
    (destroy-fiber child)
    (destroy-fiber main)))

(with-test (:name (:fiber :pin :released-on-non-local-exit))
  ;; UNWIND-PROTECT must run even when BODY transfers out non-locally.
  (let ((main (make-main-fiber)))
    (catch 'escape
      (with-fiber-pinned ()
        (throw 'escape t)))
...
(with-test (:name (:fiber :pin :released-on-error))
  (let ((main (make-main-fiber)))
    (ignore-errors
     (with-fiber-pinned ()
       (error "boom")))
...
(with-test (:name (:fiber :pin :is-per-fiber))
  ;; Pinning is per-fiber state, not global.
  (let ((main (make-main-fiber))
        (other (make-fiber (lambda ()))))
    (with-fiber-pinned ()
      (assert (fiber-pinned-p main))
      (assert (not (fiber-pinned-p other))))
    (destroy-fiber other)
    (destroy-fiber main)))
