...
int sb_fiber_ctx_gc_regs(const struct sb_fiber *f, lispobj *out, int max)
{
    ...
    if (max < n) return 0;
    int i = 0;
    out[i++] = (lispobj)f->ctx.fp;
    out[i++] = (lispobj)f->ctx.x19;
    out[i++] = (lispobj)f->ctx.x20;
    out[i++] = (lispobj)f->ctx.x21;
    out[i++] = (lispobj)f->ctx.x22;
    out[i++] = (lispobj)f->ctx.x23;
    out[i++] = (lispobj)f->ctx.x24;
    out[i++] = (lispobj)f->ctx.x25;
    out[i++] = (lispobj)f->ctx.x26;
    out[i++] = (lispobj)f->ctx.x27;
    out[i++] = (lispobj)f->ctx.x28;
    return i;
}
...
size_t sb_fiber_control_stack_size_bytes(const struct sb_fiber *f)
{
...
size_t sb_fiber_control_stack_used_bytes(const struct sb_fiber *f)
{
