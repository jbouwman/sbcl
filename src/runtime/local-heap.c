/*
 * Per-local heaps: descriptors, ownership table, TLAB switching,
 * mailboxes, and the entry points called from Lisp.  Everything that
 * needs mark-region internals lives in local-heap-gc.inc, which is
...
extern int local_heap_debug;
int local_heap_verify_after_gc;
uint32_t *block_owner;
unsigned char *ph_process_page;
unsigned char *ph_page_partial;
struct local_heap **local_heap_table;
uint32_t local_heap_table_size;
uint32_t local_heap_count;
...
static struct local_heap *table_lookup(uint32_t id, uint32_t epoch)
{
    if (id == 0 || id > local_heap_table_size) return NULL;
    struct local_heap *h = local_heap_table[id - 1];
    return (h && h->epoch == epoch && h->state == LOCAL_HEAP_LIVE) ? h : NULL;
}
...
void local_heap_init(page_index_t npages)
{
    ...
    local_heap_debug = getenv("SBCL_PH_DEBUG") != NULL;
    local_heap_verify_after_gc = getenv("SBCL_PH_VERIFY") != NULL;
}
...
static void table_register(struct local_heap *h)
{
    ignore_value(mutex_acquire(&local_heap_table_lock));
    uint32_t slot = 0;
    for (; slot < local_heap_table_size; slot++)
        if (!local_heap_table[slot]) break;
    if (slot == local_heap_table_size) {
        uint32_t newsize = local_heap_table_size ? 2 * local_heap_table_size : 64;
        struct local_heap **newtab = calloc(newsize, sizeof *newtab);
        gc_assert(newtab);
        if (local_heap_table)
            memcpy(newtab, local_heap_table,
                   local_heap_table_size * sizeof *newtab);
        struct local_heap **old = local_heap_table;
        local_heap_table = newtab;
        __atomic_store_n(&local_heap_table_size, newsize, __ATOMIC_RELEASE);
        free(old);
    }
    h->id = slot + 1;
    h->epoch = ++local_heap_next_epoch;
    local_heap_table[slot] = h;
    local_heap_count++;
    ignore_value(mutex_release(&local_heap_table_lock));
}

/* Idempotent: local_heap_release_id takes a heap out of the table
 * before anything else. */
static void table_unregister(struct local_heap *h)
{
    ignore_value(mutex_acquire(&local_heap_table_lock));
    gc_assert(h->id);
    if (local_heap_table[h->id - 1] == h) {
        local_heap_table[h->id - 1] = NULL;
        local_heap_count--;
    }
    ignore_value(mutex_release(&local_heap_table_lock));
}
...
{
    h->store_check = (flags & LOCAL_HEAP_FLAG_SIGNAL_STORES) ? LOCAL_HEAP_STORES_SIGNALED
        : (flags & LOCAL_HEAP_FLAG_RECORD_STORES) ? LOCAL_HEAP_STORES_RECORDED
        : LOCAL_HEAP_STORES_UNCHECKED;
    h->strict = (flags & LOCAL_HEAP_FLAG_STRICT) != 0;
    if (h->installed_on)
        h->installed_on->local_heap_check = h->store_check ? h : NULL;
}

struct local_heap *local_heap_create(int kind, uword_t gc_threshold,
                                         uword_t hard_limit, int flags)
{
    struct local_heap *h = calloc(1, sizeof *h);
    if (!h) return NULL;
...
static void free_descriptor(struct local_heap *h)
{
...
int local_heap_release_internal(struct local_heap *h, int force)
{
    ...
    h->state = LOCAL_HEAP_RELEASED;
    struct local_heap *frag = h->mailbox_head;
    ...
    while (frag) {
        struct local_heap *next = frag->next_in_mailbox;
        local_heap_free_pages_locked(frag);
        frag->state = LOCAL_HEAP_RELEASED;
        table_unregister(frag);
        free_descriptor(frag);
        frag = next;
    }
    local_heap_free_pages_locked(h);
 ...
 * Its slot stays taken until local_heap_release_internal has untagged
...
int local_heap_switch_in_pa(struct thread *th, struct local_heap *to)
{
...
int local_heap_switch(struct thread *th, struct local_heap *to)
{
    ph_enter_pa(th);
    int rc = local_heap_switch_in_pa(th, to);
    ph_exit_pa(th);
    return rc;
}
...
int local_heap_collect(struct local_heap *h, int full)
{
    ...
    return local_heap_collect_impl(h, sp, full);
}
...
void local_heap_run_pending_gc(os_context_t *context)
{
    ...
    funcall0(StaticSymbolFunction(LOCAL_HEAP_COLLECT_PENDING));
...
void local_heap_exhausted(struct local_heap *h, sword_t nbytes)
{
     ...
     * local_heap_take_exhausted) if the error is handled inside the
     * heap's dynamic extent. */
    local_heap_switch_in_pa(th, NULL);
    ...
    funcall2(StaticSymbolFunction(LOCAL_HEAP_EXHAUSTED_ERROR),
             available, nbytes);
...
void local_heap_check_store(lispobj value, lispobj object)
{
    ...
    funcall3(StaticSymbolFunction(HEAP_STORE_ERROR),
...
int local_heap_send(struct local_heap *dest, struct local_heap *f,
                      uint32_t sender_id)
{
    if (f->kind != LOCAL_HEAP_FRAGMENT || f->state != LOCAL_HEAP_LIVE
        || f->installed_on)
...
void local_heap_adopt(struct local_heap *dest, struct local_heap *f)
{
    sigset_t old;
    block_blockable_signals(&old);
    acquire_gc_page_table_lock();
...
int local_heap_root_offset(void)
{
    return offsetof(struct local_heap, root);
}
