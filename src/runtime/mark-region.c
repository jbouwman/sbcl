...
/* A local heap's local collection runs on the heap's own thread, at
...
/* Non-NULL on a thread while it collects a local heap. */
static _Thread_local struct ph_local_gc *ph_local;
static inline generation_index_t gen_to_collect(void);
static inline uint32_t local_heap_target_owner(void) { return ph_local ? ph_local->owner : 0; }

/* Defined in local-heap-gc.inc, included at the end of this file. */
static line_index_t local_heap_region_limit(struct alloc_region *region, line_index_t page_end);
static void local_heap_note_outgoing(lispobj object);
/* Owner of the block holding NP, 0 for global memory or outside dynamic space. */
static inline uint32_t local_heap_owner_of_native(void *np) {
...
#ifdef LISP_FEATURE_SB_LOCAL_HEAPS
  /* A local heap's region grows only through its heap's slow path,
   * which accounts for every line it claims. */
  end = local_heap_region_limit(region, end);
#endif
...
bool taggedptr_alivep_impl(lispobj object) {
  ...
  if (block_owner[address_block((void*)object)] != local_heap_target_owner()) return 1;
#else
  if (!in_dynamic_space(object)) return 1;
#endif
...
static void mark(lispobj object, lispobj *where, enum source source_type) {
  if (is_lisp_pointer(object) && in_dynamic_space(object)) {
     ...
     * traces only its heap's objects. Everything else is a leaf. */
    {
      ...
      uint32_t target = local_heap_target_owner();
      if (owner != target) {
        /* A local collection remembers the global objects it saw. */
        if (target && !owner) local_heap_note_outgoing(object);
        if (local_heap_check_refs && source_object && owner) {
          uint32_t src = local_heap_owner_of_native(source_object);
          if (src != owner) local_heap_note_violation(source_object, where, object);
        }
        return;
      }
      ...
      if (local_heap_debug) {
        ...
        if (bad) {
          fprintf(stderr, "ph BAD MARK: obj %p (word %lx) page %d type %x owner %u free %d "
                  ...
                  source_object ? local_heap_owner_of_native(source_object) : 0,
                  target, gc_active_p);
          if (source_object) {
...
#ifdef LISP_FEATURE_SB_LOCAL_HEAPS
    /* Symbols never live in a local heap. */
    if (ph_local) return;
#endif
...
#ifdef LISP_FEATURE_SB_LOCAL_HEAPS
    if (block_owner[address_block((void*)obj)] != local_heap_target_owner()) return;
#endif
    set_mark_bit(obj);
 ...
 * local-heap collection, which scans only that heap's pages. */
 ...
 * page I.  A local heap's collection passes the runs of blocks it
...
#ifdef LISP_FEATURE_SB_LOCAL_HEAPS
  if (local_heap_count)
    lose("%d local heap(s) still exist; release them before saving a core",
         local_heap_count);
#endif
  for (page_index_t p = 0; p < page_table_pages; p++) {
