...
fiber also becomes the thread's current fiber; otherwise the thread is
left with no current fiber, and WITH-CURRENT-FIBER installs the new one
where it is needed.  NAME is a string label used by PRINT-OBJECT.

Signals CURRENT-FIBER-ERROR if the thread already has a current fiber:
the code is then running on that fiber's stack, not the thread's."
...
it was not current already."
  (declare (type fiber fiber))
  (let ((existing (current-fiber)))
    (cond ((eq fiber existing) nil)
          ((fiber-released-p fiber)
           (error 'dead-fiber-error :fiber fiber))
          ((not (eq (fiber-thread fiber) sb-thread:*current-thread*))
           (error 'fiber-thread-mismatch-error :fiber fiber :role :current))
          (existing
           (error 'current-fiber-error
                  :fiber existing :operation 'with-current-fiber))
          ((not (fiber-main-p fiber))
           (error "~S is not a main fiber" fiber))
...
(defmacro with-current-fiber ((fiber) &body body)
  "Make FIBER the calling thread's current fiber for the dynamic extent
of BODY, and leave the thread with no current fiber on exit.  FIBER must
...
than by writing the thread slot."
  (let ((f (gensym "FIBER"))
    ...
    `(let* ((,f ,fiber)
            (,installed (%enter-current-fiber ,f)))
