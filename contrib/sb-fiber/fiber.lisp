...
RELEASE-FIBER is only needed if you want to reclaim resources sooner."
  (unless *current-fiber*
    (error "MAKE-FIBER: calling thread has no main fiber.  ~
            Call MAKE-MAIN-FIBER or wrap the work in WITH-FIBER-THREAD ~
            before allocating fibers."))
...
fiber wrapper)."
  ...
  (if (fiber-released-p fiber)
      0
      (let* ((c (fiber-c (fiber-sap fiber)))
             (b (sb-fiber-c-bs-base c))
             (e (sb-fiber-c-bs-end c)))
...
binding-stack start).  Works for both worker and main fibers."
  ...
  (if (fiber-released-p fiber)
      0
      (let* ((c        (fiber-c (fiber-sap fiber)))
             (bsp      (sb-fiber-c-bsp c))
             (bs-start (sb-fiber-c-bs-start c)))
        (if (or (zerop bsp) (zerop bs-start))
            0
            (max 0 (- bsp bs-start))))))

(defun %switch-fiber (from to values update-return-p)
  ...
  (let* ((self *current-fiber*)
    ...
    (unless target
      (error "yield-fiber: ~S has no return fiber (never resumed)"
             self))
    (%switch-fiber self target values nil)))
