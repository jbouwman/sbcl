...
:FROM or :TO, or :CURRENT for WITH-CURRENT-FIBER.")
  (:report (lambda (c stream)
...
(define-condition current-fiber-error (fiber-error)
  ((operation :initarg :operation :initform nil
              :reader current-fiber-error-operation))
  (:documentation
   "An operation that must run on the thread's own stack, with no
current fiber, was attempted while FIBER was current.")
  (:report (lambda (c stream)
             (format stream "~@[~(~A~): ~]thread already has current fiber ~S"
