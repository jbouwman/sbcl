...
;;; The scheduler drains runnables, then rebuilds a pollfd array from
;;; *WAITERS* and blocks in poll(2).  When the scheduler does
;;; (fiber-switch *scheduler* f) it implicitly sets f's return-fiber
;;; to *scheduler*, so a fiber yields back with a plain (fiber-yield).
(defvar *scheduler*)
(defvar *runnable*)
(defvar *waiters*)
